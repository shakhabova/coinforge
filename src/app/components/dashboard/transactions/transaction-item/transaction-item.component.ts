...
export class TransactionItemComponent {
	...
	isPositiveOpr = computed(() =>
		['IN', 'F2C', 'C2C'].includes(this.transaction().type),
	);

	private amountSum = computed(() =>
		this.isPositiveOpr()
			? this.transaction().amount
			: this.transaction().amountInSenderCurrency,
	);
	private amountCurr = computed(() =>
		this.isPositiveOpr()
			? this.transaction().currencyTo
			: this.transaction().currencyFrom,
	);
	amount = computed(
		() =>
			`${this.isPositiveOpr() ? '+' : '-'} ${this.amountSum()} ${this.amountCurr()}`,
	);
}
